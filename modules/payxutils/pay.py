...
from telethon.tl.types import InputMediaInvoice, Invoice, LabeledPrice, KeyboardButtonBuy, KeyboardButtonCopy, DataJSON
...
def setup_donate_handler(app):
            ...
            if user_id in admin_ids or user_id == DEVELOPER_USER_ID:
                ...
                try:
                    result = await app(RefundStarsChargeRequest(
                        user_id=types.InputUser(user_id=refund_user_id, access_hash=(await app.get_entity(refund_user_id)).access_hash),
    ...
    async def raw_update_handler(update):
        if isinstance(update, UpdateBotPrecheckoutQuery):
            ...
            try:
                await app(functions.messages.SetBotShippingResultsRequest(
                    query_id=update.query_id,
                    shipping_options=[]
                ))
        ...
        elif isinstance(update, UpdateNewMessage) and isinstance(update.message, types.MessageService) and isinstance(update.message.action, MessageActionPaymentSentMe):
            ...
            try:
                if update.message.from_id and hasattr(update.message.from_id, 'user_id'):
                    ...
                    possible_user_ids = [u.id for u in update.users if u.id > 0]
                ...
                if await banned_users.banned_users.find_one({"user_id": user_id}):
                    ...
                    return
                if isinstance(update.message.peer_id, types.PeerUser):
                ...
                elif isinstance(update.message.peer_id, types.PeerChannel):
                    chat_id = update.message.peer_id.channel_id
                else:
                    chat_id = user_id
                if not chat_id:
                    raise ValueError(f"Invalid chat_id ({chat_id})")
                user = await app.get_entity(user_id) if user_id else None
                full_name = get_display_name(user) or "Unknown" if user else "Unknown"
                ...
                if chat_id:
                    user_name = get_display_name(await app.get_entity(DEVELOPER_USER_ID))
                    await app.send_message(
                        entity=chat_id,
